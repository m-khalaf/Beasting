...
const Home = () => {
  ...
  useEffect(() => {
    ...
    Box.prototype.update = function() {
      ...
      if (this.x > canvas.width / 2 - 4000 && this.x < canvas.width / 2 + 400 && this.y > canvas.height / 2 - 100 && this.y < canvas.height / 2 + 100) {
        this.x = Math.floor(Math.random() * (canvas.width - 50));
        this.y = Math.floor(Math.random() * (canvas.height - 50));
      }
    };
  ...
  return (
    <div style={{ color: '#c4b6a1', position: 'relative', backgroundColor: '#efd9b4', height: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <canvas ref={canvasRef} style={{ position: 'absolute', top: 0, left: 0 }} width={window.innerWidth} height={window.innerHeight} />
